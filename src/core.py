...
def load_cmapss_data(data_path: Path, sep: str='\\s+') -> pd.DataFrame:
    """Load CMAPSS dataset."""
...
def plot_health_index(df: pd.DataFrame, unit_id: int, distance_col: str, threshold: float, output_path: Path, plot: bool = False):
    ...
    ax.plot(unit_data['time'], unit_data[distance_col], color='#4A90A4', linewidth=1.2, alpha=0.8)
    ax.axhline(threshold, color='#D4A574', linestyle='--', linewidth=1.2, label='Threshold')
    ax.set_xlabel('Cycle')
